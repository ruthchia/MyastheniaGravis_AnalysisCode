...
manhattan="Meta.USmerged.Itals.EOMG.GWAS.TWAS.mirror.filteredMAFoverall.manhattan_v0.jpeg"
...
plot_v0 <- ggplot(don, aes(x=BPcum, y=-log10(TWAS.P))) +
            ...
            scale_y_continuous(limits = c(0,maxY0), breaks = seq(0, maxY0, by = 5), expand = c(0, 0) ) +
            ggtitle(paste("Manhattan Plot:\n","Meta.USmerged.Itals.EOMG.TWAS.hg38.maf(overall)>0.01","\n(",NoCases," overall, ",NoControls," controls)", sep="")) +
            xlab("Chromosome") +
            ylab(expression(-log[10](P))) +
            geom_point(data=subset(don, is_highlight_subsig=="yes"), color="#e69f00", size=0.6) +
            ...
            annotate("text", label = "EARLY ONSET", color="grey50", hjust = 0, x = minX1+100, y = 33, size=2.5) +
            annotate("text", label = "Skeletal muscle", color="grey50", hjust = 0, x = minX1+100, y = 29.5, size=2.5)
...
plot_v3 <- ggplot(don3, aes(x=BPcum, y=-log10(TWAS.P))) +
            ...
            scale_alpha_manual(guide='none', values = list("FALSE" = 0.8, "TRUE" = 0.2)) +
            scale_x_continuous(limits = c(minX1,maxX1),label = axisdf1$CHROM, breaks= axisdf1$center, position="bottom" ) +
            scale_y_continuous(limits = c(0,maxY3), breaks = seq(0, maxY3, by = 5), expand = c(0, 0) ) +
            ggtitle(paste("Manhattan Plot:\n","Meta.USmerged.Itals.EOMG.TWAS.hg38.maf(overall)>0.01","\n(",NoCases," overall, ",NoControls," controls)", sep="")) +
            xlab("Chromosome") +
            ylab(expression(-log[10](P))) +
            geom_point(data=subset(don3, is_highlight_subsig=="yes"), color="#e69f00", size=0.6) +
            ...
            annotate("text", label = "EARLY ONSET", color="grey50", hjust = 0, x = minX1+100, y = 43, size=2.5) +
            annotate("text", label = "Whole blood", color="grey50", hjust = 0, x = minX1+100, y = 38.5, size=2.5)
...
plot_v4 <- ggplot(don4, aes(x=BPcum, y=-log10(TWAS.P))) +
            ...
            scale_alpha_manual(guide='none', values = list("FALSE" = 0.8, "TRUE" = 0.2)) +
            scale_x_continuous(limits = c(minX1,maxX1),label = axisdf1$CHROM, breaks= axisdf1$center, position="bottom" ) +
            scale_y_continuous(limits = c(0,maxY4), breaks = seq(0, maxY4, by = 5), expand = c(0, 0) ) +
            ggtitle(paste("Manhattan Plot:\n","Meta.USmerged.Itals.EOMG.TWAS.hg38.maf(overall)>0.01","\n(",NoCases," overall, ",NoControls," controls)", sep="")) +
            xlab("Chromosome") +
            ylab(expression(-log[10](P))) +
            geom_point(data=subset(don4, is_highlight_subsig=="yes"), color="#e69f00", size=0.6) +
            ...
            annotate("text", label = "EARLY ONSET", color="grey50", hjust = 0, x = minX1+100, y = 43, size=2.5) +
            annotate("text", label = "Tibial nerve", color="grey50", hjust = 0, x = minX1+100, y = 38.5, size=2.5)

blank <- grid.rect(gp=gpar(col="white"))
all <- grid.arrange(plot_v0,plot_v3,plot_v4,ncol = 1)

# Save manhattan plot
ggsave(paste("Meta.USmerged.Itals.EOMG.TWAS.only",".filteredMAFoverall.manhattan_v0.jpeg", sep = ""), plot = all, device = "jpeg", scale = 1, width = 9, height = 9, units = "in", dpi = 300, limitsize = TRUE)
ggsave(paste("Meta.USmerged.Itals.EOMG.TWAS.only",".filteredMAFoverall.manhattan_v0.pdf", sep = ""), plot = all, device = "pdf", scale = 1, width = 9, height = 9, units = "in", dpi = 300, limitsize = TRUE)
...
manhattan="Meta.USmerged.Itals.EOMG.TWAS.only.filteredMAFoverall.manhattan_v0.jpeg"
display(Image.open(manhattan))
```
...
```bash
%%bash
awk '{print $8,$10}' META_USmerged.Itals.EOMG.UNRELATED.hg38.filteredDirection.HetISq80MAFoverall001.GTEx.MGtissues.PERM.topSummStats | sort -k1 | uniq | column -t
```

    BEST.GWAS.ID  EQTL.ID
    rs10484439    rs16891235
    rs10484439    rs9366653
    rs1150753     rs3101018
    rs1150753     rs497309
    rs13198716    rs9379851
    rs13218875    rs1225591
    rs13218875    rs13218875
    rs13218875    rs1778508
    rs13218875    rs853685
    rs2854275     rs35404844
    rs3094005     rs1150753
    rs3094005     rs3101018
    rs3094005     rs3130560
    rs3094005     rs9332704
...
echo "rs10484439	rs16891235	0.856	0.5174" >> GWAS.EQTL.LDinfo.txt
echo "rs10484439	rs9366653	0.9184	0.4908" >> GWAS.EQTL.LDinfo.txt
echo "rs1150753	rs3101018	0.9712	0.9297" >> GWAS.EQTL.LDinfo.txt
echo "rs1150753	rs497309	1	0.9716" >> GWAS.EQTL.LDinfo.txt
echo "rs13198716	rs9379851	0.9669	0.5356" >> GWAS.EQTL.LDinfo.txt
echo "rs13218875	rs1225591	0.9823	0.2459" >> GWAS.EQTL.LDinfo.txt
echo "rs13218875	rs13218875	1	1" >> GWAS.EQTL.LDinfo.txt
echo "rs13218875	rs1778508	0.9082	0.4977" >> GWAS.EQTL.LDinfo.txt
echo "rs13218875	rs853685	0.8926	0.4671" >> GWAS.EQTL.LDinfo.txt
echo "rs2854275	rs35404844	0.5268	0.0816" >> GWAS.EQTL.LDinfo.txt
echo "rs3094005	rs1150753	0.8853	0.7122" >> GWAS.EQTL.LDinfo.txt
echo "rs3094005	rs3101018	0.9273	0.7704" >> GWAS.EQTL.LDinfo.txt
echo "rs3094005	rs9332704	1	0.001" >> GWAS.EQTL.LDinfo.txt
echo "rs3094005	rs3130560	1	0.0345" >> GWAS.EQTL.LDinfo.txt
echo "rs3129063	rs3129063	1	1" >> GWAS.EQTL.LDinfo.txt
echo "rs3130557	rs3130557	1	1" >> GWAS.EQTL.LDinfo.txt
echo "rs3130557	rs3130564	1	0.4906" >> GWAS.EQTL.LDinfo.txt

cat GWAS.EQTL.LDinfo.txt | column -t
```

    BEST.GWAS.ID  EQTL.ID     LD.Dprime  LD.R2
    rs10484439    rs16891235  0.856      0.5174
    rs10484439    rs9366653   0.9184     0.4908
    rs1150753     rs3101018   0.9712     0.9297
    rs1150753     rs497309    1          0.9716
    rs13198716    rs9379851   0.9669     0.5356
    rs13218875    rs1225591   0.9823     0.2459
    rs13218875    rs13218875  1          1
    rs13218875    rs1778508   0.9082     0.4977
    rs13218875    rs853685    0.8926     0.4671
    rs2854275     rs35404844  0.5268     0.0816
    rs3094005     rs1150753   0.8853     0.7122
    rs3094005     rs3101018   0.9273     0.7704
    rs3094005     rs9332704   1          0.001
    rs3094005     rs3130560   1          0.0345
    rs3129063     rs3129063   1          1
    rs3130557     rs3130557   1          1
    rs3130557     rs3130564   1          0.4906
...
data <- fread("META_USmerged.Itals.EOMG.UNRELATED.hg38.filteredDirection.HetISq80MAFoverall001.GTEx.MGtissues.PERM.topSummStats",header=T) %>% select(-FILE)
...
write.table(data4,"META_USmerged.Itals.EOMG.UNRELATED.hg38.filteredDirection.HetISq80MAFoverall001.GTEx.MGtissues.topSummStats.AddINFO.txt",sep="\t",quote=F,row.names=F,col.names=T)
...
```python
!head -n 1 META_USmerged.Itals.EOMG.UNRELATED.hg38.filteredDirection.HetISq80MAFoverall001.GTEx.MGtissues.topSummStats.AddINFO.txt | tr '\t' '\n' | cat -n
```
         ...
         6	HSQ
         7	BEST.GWAS.ID
         8	BEST.GWAS.ID.MarkerName
         9	EffectAllele
        10	OtherAllele
        11	BEST.GWAS.ID.maf_EA.cases
        12	BEST.GWAS.ID.maf_EA.ctrls
        13	BEST.GWAS.Z
        14	EQTL.ID
        15	EQTL.ID.MarkerName
        16	EQTL.ID.maf_EA.cases
        17	EQTL.ID.maf_EA.ctrls
        18	EQTL.Z
        19	EQTL.GWAS.Z
        20	LD.Dprime
        21	LD.R2
...
```python
!awk 'NR==1;NR>1 {if($14 != "NA") print}' META_USmerged.Itals.EOMG.UNRELATED.hg38.filteredDirection.HetISq80MAFoverall001.GTEx.MGtissues.topSummStats.AddINFO.txt | grep -v ".BLOOD.RNAARR" | cut -f1-3,14-25 | grep -v "NA" > META_USmerged.Itals.EOMG.UNRELATED.hg38.filteredDirection.HetISq80MAFoverall001.GTEx.MGtissues.topSummStats.AddINFO.TIDYforPUBLICATION.txt
```


```python
import pandas as pd
pd.read_csv("META_USmerged.Itals.EOMG.UNRELATED.hg38.filteredDirection.HetISq80MAFoverall001.GTEx.MGtissues.topSummStats.AddINFO.TIDYforPUBLICATION.txt",sep="\t")
```
